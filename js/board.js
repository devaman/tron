...
Board.prototype.clearSegment = function(id,self){
  this.players[id].segments.map((coord) => {
    this.board[coord.i][coord.j] = 0;
    var coordIdx = (coord.i * self.board.dimX) + coord.j;
    self.$li.eq(coordIdx).css("background", `transparent`);
    self.$li.eq(coordIdx).css("box-shadow", `none`);
  })
   this.players[id].segments = [];
}
